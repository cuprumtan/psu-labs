...
class Streebog512:
    ...
    def padding(self, last, N, h, Sigma):
        if len(last) < 64:
            padding = [0] * (64 - len(last))
            padding[-1] = 1
            padded_message = padding + last
        h = self.gN(N, padded_message, h)
        N_len = [0] * 64
        N_len[63] = (len(last) * 8) & 0xff
        N_len[62] = (len(last) * 8) >> 8
        N = self.MOD2512(N, N_len)
        Sigma = self.MOD2512(Sigma, padded_message)
        return h, N, Sigma

    def get_hash(self, message, no_pad=False):
        N = [0] * 64
        Sigma = [0] * 64
        ...
        while length_bits >= 512:
            ...
            h = self.gN(N, tmp, h)
            N = self.MOD2512(N, N_512)
            Sigma = self.MOD2512(Sigma, tmp)
            length_bits -= 512
            i += 1
        ...
        if len(last) == 0 and no_pad:
            ...
            h, N, Sigma = self.padding(last, N, h, Sigma)

        N_0 = [0] * 64
        h = self.gN(N_0, N, h)
        h = self.gN(N_0, Sigma, h)
